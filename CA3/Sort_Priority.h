void Sort_Priority(Task *head,char path[40])
{
    Task *current;
    current = head;
    ...
    for(int i=0;i<nT;i++)
    {
        ...
        while((current2->next != NULL) && (current2->next->next != NULL))
        {
            ...
            flag = 0;
            if((*(current2->priority) == 'L'  || *(current2->priority) == 'l') && (*(current2->next->priority) == 'm'  || *(current2->next->priority) == 'M')) flag =1;
            if((*(current2->priority) == 'm'  || *(current2->priority) == 'M') && (*(current2->next->priority) == 'H'  || *(current2->next->priority) == 'h')) flag =1;
            if((*(current2->priority) == 'L'  || *(current2->priority) == 'l') && (*(current2->next->priority) == 'h'  || *(current2->next->priority) == 'H')) flag =1;
